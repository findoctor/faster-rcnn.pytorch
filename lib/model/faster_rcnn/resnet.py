...
class ResNet(nn.Module):
  def __init__(self, block, layers, num_classes=1000):
    ...
    self.layer4 = self._make_layer(deformBottleneck, 512, layers[3], stride=1)
  ...
  def forward(self, x):
    ...
    x = self.layer3(x)
    x = self.layer4(x)
    # Use new layer 4 with deformable-conv
...
'''
resnet_final = resnet(20, num_layers=101, pretrained=False, class_agnostic=False)
print(resnet_final)

y = resnet_final(torch.randn(10, 3, 64, 64))
print('final output dimension of deform_ResNet:')
print(y.size())
'''
